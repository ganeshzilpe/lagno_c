...
void display(char board[][SIZE])
{
	...
	setcolor(6);
	setcolor(10);
	for(r=0, i=a/2-(20*SIZE);i<a/2+(20*SIZE);r++,i+=40)
		for( c=0,j=b/2-(20*SIZE);j<b/2+(20*SIZE);c++,j+=40)
		{
			switch(board[r][c])
			{	case '@':
...
int best_move(char board[][SIZE], int moves[][SIZE], char player)
{
   ...
   char new_board[SIZE][SIZE] = { 0 };  /* copy of board    */
